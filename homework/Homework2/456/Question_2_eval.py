...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        k = 0
        # saver = tf.train.import_meta_graph(r'.\data\model.ckpt.meta')
        ...
        for epoch in range(epoch_times):
            ...
            for i in range(iter_per_epoch):
                ...
                if i % 100 == 0:
                    ...
                    print('Training loss =', values[0])
                     
                    values = sess.run([loss, accuracy], feed_dict = {x: testing_samples[:1000], y: testing_labels[:1000], select_prob: 1.0})
                    loss_testing_list.append(values[0])
                    accuarcy_testing_list.append(values[1])
                    print('Testing accuracy =', values[1])
